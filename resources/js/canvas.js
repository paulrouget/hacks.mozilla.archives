...
(function(){
  ...
  var extend = {
    getIdx: function(aX, aY) {
      if (aX >= this.canvas.width ||
          aY >= this.canvas.height ||
          aX < 0 ||
          aY < 0) {
        throw ("value out of range");
      }
      return 4 * (aX + aY * this.canvas.width);
    },
    // Circle methods
    circle: function(aX, aY, aDiameter){
      this.ellipse(aX, aY, aDiameter, aDiameter);
    },
    fillCircle: function(aX, aY, aRadius){
      this.beginPath();
      this.circle(aX, aY, aRadius * 2);
      this.fill();
    },
